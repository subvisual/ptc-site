...
function Field({
	...
	const base: React.CSSProperties = {
		...
		background: readOnly ? SITE.limestone : "#fff",
		outline: "none",
	...
	return (
		...
		>
			...
			>
				...
				{error && (
					<div style={{ color: SITE.red, fontSize: 12, marginBottom: 12 }}>
						{error}
					</div>
				)}
				<button
...
function EventsSection({ communities }: { communities: ApiCommunity[] }) {
	...
	const commMap = Object.fromEntries(
		communities.map((c) => [c.notionId, c.name]),
	);
		...
		<div>
			...
			>
				<div style={{ display: "flex", gap: 12, alignItems: "center" }}>
					...
					{pending.length > 0 && (
						<Badge label={`${pending.length} pendentes`} color={SITE.ochre} />
					)}
					{msg && (
						<span style={{ fontSize: 12, color: SITE.green }}>{msg}</span>
					)}
				</div>
				<button
			...
			{loading ? (
				<div style={{ color: SITE.mute, fontSize: 13, padding: 20 }}>
					A carregar…
				</div>
			) : (
				<>
			...
			{editing && (
				<Modal title="Editar evento" onClose={() => setEditing(null)}>
					...
					<Field
						...
						onChange={(v) => setEditing((p) => p && { ...p, description: v })}
						multiline
					/>
					<Field
						label="Local"
						value={editing.venue}
						onChange={(v) => setEditing((p) => p && { ...p, venue: v })}
					/>
					<Field
						label="Data (ISO)"
						value={editing.date ?? ""}
						onChange={(v) => setEditing((p) => p && { ...p, date: v })}
						placeholder="2026-06-11T19:30"
					/>
					<Field
						label="Região"
						value={editing.region}
						onChange={(v) => setEditing((p) => p && { ...p, region: v })}
					/>
					<Field
						label="URL do evento"
						value={editing.eventUrl}
						onChange={(v) => setEditing((p) => p && { ...p, eventUrl: v })}
					/>
...
function CommunitiesSection() {
	...
	async function toggleApprove(c: ApiCommunity) {
		const updated = await api.updateCommunity(c.notionId, {
			approved: !c.approved,
		});
		setComms((prev) =>
			prev.map((x) => (x.notionId === updated.notionId ? updated : x)),
		);
	}
	...
	return (
		<div>
			...
			>
				...
				{pending.length > 0 && (
					<Badge
						label={`${pending.length} novas submissões`}
						color={SITE.ochre}
					/>
				)}
			...
			{editing && (
				<Modal title="Editar comunidade" onClose={() => setEditing(null)}>
					...
					<Field
						label="Slug (ID)"
						value={editing.slug}
						onChange={(v) => setEditing((p) => p && { ...p, slug: v })}
					/>
					<Field
						label="Descrição"
						value={editing.description}
					...
					<Field
						...
						onChange={(v) => setEditing((p) => p && { ...p, members: v })}
						placeholder="1.2k"
					/>
					<Field
						label="Fundada"
						value={editing.founded?.toString() ?? ""}
						onChange={(v) =>
							setEditing((p) => p && { ...p, founded: parseInt(v) || null })
						}
					/>
					<Field
						label="URL da comunidade"
						value={editing.communityPage}
						onChange={(v) => setEditing((p) => p && { ...p, communityPage: v })}
					/>
					<Field
						label="URL do logo"
						value={editing.logoUrl}
						onChange={(v) => setEditing((p) => p && { ...p, logoUrl: v })}
					/>
					<div style={{ display: "flex", gap: 12, marginTop: 8 }}>
						<button
							onClick={saveEdit}
	...
	return (
		...
		>
			<div>
				...
				>
					{comm.region} · {comm.members || "—"} membros · desde{" "}
					{comm.founded ?? "—"}
				</div>
				{comm.description && (
					<div style={{ fontSize: 12, color: SITE.mute, marginTop: 4 }}>
						{comm.description}
					</div>
				)}
			</div>
			<div style={{ display: "flex", gap: 8, alignItems: "center" }}>
				...
				<button
					onClick={() => onDelete(comm.notionId)}
					style={{
						background: "none",
						border: `1px solid ${SITE.red}40`,
						padding: "4px 10px",
...
function LeadersSection({ communities }: { communities: ApiCommunity[] }) {
	...
	const load = useCallback(async () => {
		setLoading(true);
		try {
			setLeaders(await api.getLeaders(false));
		} catch (e: any) {
			setMsg({
				text: "Erro: " + (e.message ?? "falha ao carregar"),
				error: true,
			});
		} finally {
			setLoading(false);
	...
	async function toggle(l: ApiLeader) {
		try {
			...
			setMsg({
				text: "Erro: " + (e.message ?? "falha ao guardar"),
				error: true,
			});
		}
	}
	...
	function Row({ l }: { l: ApiLeader }) {
		...
		return (
			...
			>
				<div>
					<div style={{ fontWeight: 600, fontSize: 14, color: SITE.ink }}>
						{l.email}
					</div>
	...
	return (
		<div>
			...
			>
				...
				{pending.length > 0 && (
					<Badge label={`${pending.length} por aprovar`} color={SITE.ochre} />
				)}
				{msg.text && (
					<span
						style={{ fontSize: 12, color: msg.error ? SITE.red : SITE.green }}
					>
						{msg.text}
					</span>
				)}
...
function ConfigSection() {
	...
	if (!config)
		return <div style={{ color: SITE.mute, fontSize: 13 }}>A carregar…</div>;
	...
	function updateFaq(i: number, field: "q" | "a", val: string) {
		const faqs = config!.faqs.map((f: any, idx: number) =>
			idx === i ? { ...f, [field]: val } : f,
		);
		set("faqs", faqs);
	}
	...
	return (
		<div>
			...
			>
				{msg && (
					<span style={{ fontSize: 12, color: SITE.green, marginRight: 16 }}>
						{msg}
					</span>
				)}
				<button
			...
			<SectionBlock title="Formulários">
				<Field
					label="URL Submit yours"
					value={config.notionFormUrl ?? ""}
					onChange={(v) => set("notionFormUrl", v)}
					placeholder="https://..."
				/>
				<Field
					label="URL form de contacto"
					value={config.contactFormUrl ?? ""}
					onChange={(v) => set("contactFormUrl", v)}
					placeholder="https://..."
				/>
			</SectionBlock>
			...
			<SectionBlock title="Links & Redes Sociais">
				<Field
					label="Newsletter"
					value={config.newsletterUrl ?? ""}
					onChange={(v) => set("newsletterUrl", v)}
					placeholder="https://..."
				/>
				<Field
					label="WhatsApp"
			...
			<SectionBlock title="FAQs">
						...
						>
							...
							<button
								onClick={() =>
									set(
										"faqs",
										config.faqs.filter((_: any, idx: number) => idx !== i),
									)
								}
								style={{
									background: "none",
				...
				<button
					onClick={() =>
						set("faqs", [...(config.faqs ?? []), { q: "", a: "" }])
					}
					style={{
						border: `1px dashed ${SITE.rule}`,
						padding: "10px 20px",
...
function PortalSection() {
	...
	async function send() {
		...
		try {
			...
			setResult({
				ok: true,
				msg: `Link enviado para ${email} — comunidades: ${r.communities.join(", ")}`,
			});
			setEmail("");
		} catch (e: any) {
	...
	return (
		<div>
			<SectionBlock title="Enviar magic link">
				...
				>
					Envia um link de acesso ao dashboard de comunidade para um community
					manager registado no CRM. O link é válido por 24 horas.
				</p>
				<Field
...
export function Admin({ onExit }: AdminProps) {
	...
	return (
		<div style={{ background: SITE.paper, minHeight: "100vh", ...S }}>
					...
					{navItems.map((item) => (
						<button
							...
							style={{
								...
								fontSize: 14,
								color: section === item.key ? SITE.ink : SITE.mute,
								...S,
							}}
						>
							{item.label}
			...
			{/* Content */}
			<div
				style={{ maxWidth: 900, margin: "0 auto", padding: "40px 48px 80px" }}
			>
