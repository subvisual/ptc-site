...
function toDisplayEvent(
	e: ApiEvent,
	communityMap: Map<string, ApiCommunity>,
): DisplayEvent {
		...
		<div style={{ marginTop: 32 }}>
			...
			>
				{cells.map((day, i) => {
					if (day === null)
						return (
							<div key={i} style={{ background: T.paperWarm, minHeight: 80 }} />
						);
					...
					return (
						<div
							key={i}
							onClick={() =>
								dayEvents.length > 0 && setSelectedDay(isSelected ? null : day)
							}
							style={{
			...
			{selectedDay !== null && selectedEvents.length > 0 && (
				...
				>
					...
					>
						{MONTHS[month]} {selectedDay} · {selectedEvents.length} event
					...
					{Object.entries(CITY_POSITIONS).map(([city, pos]) => {
						...
						return (
							<g
								key={city}
								onClick={() =>
									isActive && setSelectedCity(isSelected ? null : city)
								}
			...
			<div>
				{selectedCity ? (
					<>
						...
						>
							{selectedCity} · {selectedEvents.length} event
							{selectedEvents.length !== 1 ? "s" : ""}
							<span
...
export function Events({ onNavigate, onOpenSubmit }: EventsProps) {
	...
	const filtered = useMemo(() => {
		return events.filter((e) => {
			const cityMatch =
				selectedCities.length === 0 || selectedCities.includes(e.region);
			const themeMatch =
	...
	return (
		...
		>
			...
			{view === "list" && (
				<div
					className="page-pad"
					style={{ borderBottom: `1px solid ${T.rule}`, display: "flex" }}
				>
