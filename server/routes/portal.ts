...
import { SESSION_SECRET, SESSION_TTL_MS } from "../lib/env.js";
import { authLimiter } from "../lib/ratelimit.js";
import { serverError } from "../lib/http.js";
import { validate, magicLinkInput } from "../lib/validation.js";
...
function verifyPortalSession(cookie: string): PortalSession | null {
	...
	try {
		...
		// Enforce the 8h lifetime server-side rather than trusting the cookie maxAge.
		if (
			typeof session.iat !== "number" ||
			Date.now() - session.iat > SESSION_TTL_MS
		)
			return null;
		return session;
	} catch {
...
portalRouter.post("/magic-link", authLimiter, requireAuth, async (req, res) => {
	try {
		const parsed = validate(magicLinkInput, req.body);
		...
		if (!leader || !leader.communityIds.length) {
			return res
				.status(404)
				.json({ error: "Nenhuma comunidade associada a este email." });
		}
